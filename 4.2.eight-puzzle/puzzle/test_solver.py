"""
Unit tests for PuzzleSolver.solve() (Part 2).

Solution validity and optimality are checked with a small breadth-first
search over Puzzle.get_neighboring_states(), independent of the solver, so
these tests do not assume any particular heuristic implementation, only
that the returned solution is a valid, optimal path from start to goal.
All puzzles used here are solvable; PuzzleSolver may assume the goal is
always reachable from the start.
"""
...
def _bfs_optimal_length(start: Puzzle, goal: Puzzle) -> int | None:
    if start == goal:
        return 0
...
def _assert_valid_solution(solution, start, goal):
    assert solution[0] == start
    assert solution[-1] == goal
    for current, next_state in zip(solution, solution[1:]):
        assert next_state in current.get_neighboring_states()


ONE_MOVE_AWAY = Puzzle.from_rows(
    [
        [1, 2, 3],
        [4, 5, 6],
        [7, 0, 8],
...
TWO_MOVES_AWAY = Puzzle.from_rows(
    [
        [1, 2, 3],
        [4, 0, 6],
        [7, 5, 8],
...
THREE_MOVES_AWAY = Puzzle.from_rows(
    [
        [1, 2, 3],
        [0, 4, 6],
        [7, 5, 8],
...
class TestSolvesTrivialPuzzle:
    def test_already_at_goal_returns_single_state_solution(self):
        ...
        solution = solver.solve(GOAL_STATE, GOAL_STATE)
...
class TestSolvesSimplePuzzles:
    def test_solution_is_valid_and_optimal_for_a_one_move_puzzle(self):
        ...
        solution = solver.solve(ONE_MOVE_AWAY, GOAL_STATE)
    ...
    def test_solution_is_valid_and_optimal_for_a_two_move_puzzle(self):
        ...
        solution = solver.solve(TWO_MOVES_AWAY, GOAL_STATE)
    ...
    def test_solution_is_valid_and_optimal_for_a_multi_move_puzzle(self):
        ...
        solution = solver.solve(THREE_MOVES_AWAY, GOAL_STATE)
